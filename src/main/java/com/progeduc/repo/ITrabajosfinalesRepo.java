...
public interface ITrabajosfinalesRepo  extends CrudRepository<Trabajosfinales,Integer>{
	...
	@Transactional
	@Modifying	
	@Query(value="update Trabajosfinales p set p.nota_nacional = 0,p.puesto_nacional=0,p.estadonacional=3 WHERE p.id = ?1",nativeQuery = true)
	...
	@Query(value="select DISTINCT tf.* from trabajosfinales tf "
    		...
    		+ "and tf.anio = EXTRACT(YEAR FROM sysdate) AND tf.puesto = 1 and tf.estado = 1 "
	...
	@Query(value="select DISTINCT tf.* from trabajosfinales tf "
    		...
    		+ "and tf.anio = EXTRACT(YEAR FROM sysdate) AND tf.puesto_nacional > 0 AND tf.empate_nacional=1 and tf.estado = 1 "
    		+ "order by tf.nota_nacional desc ",nativeQuery = true)
	...
	@Query(value="select DISTINCT tf.* from trabajosfinales tf "
    		...
    		+ "and tf.anio = EXTRACT(YEAR FROM sysdate) AND tf.puesto_nacional= ?3 and tf.estado = 1 "
    		+ "order by tf.nota_nacional desc ",nativeQuery = true)
