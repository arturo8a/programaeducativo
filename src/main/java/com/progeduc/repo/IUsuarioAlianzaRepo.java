...
public interface IUsuarioAlianzaRepo extends CrudRepository<UsuarioAlianza,Integer>{
	...
	@Query(value="SELECT U.* FROM USUARIO_ALIANZA U WHERE U.usuario_autoridad = ?1 and U.password_autoridad=?2 and U.estado=1 and U.anio = EXTRACT(YEAR FROM sysdate) ",nativeQuery = true)
	...
	@Query(value="SELECT TB1.* FROM USUARIO_ALIANZA TB1 WHERE TB1.anio = ?1 ",nativeQuery = true)
	List<UsuarioAlianza> listarPorAnio(Integer anio);
	
	@Query(value="SELECT TB1.* FROM USUARIO_ALIANZA TB1 WHERE TB1.odsid = ?1 and TB1.anio = ?2 ",nativeQuery = true)
