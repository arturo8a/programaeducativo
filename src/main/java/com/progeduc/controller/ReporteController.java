...
public class ReporteController {
	...
	public ByteArrayInputStream reporteexcelnotasevaluador(Integer ods,Integer anio,Integer categoria,Integer nivel,HttpSession ses)   {
		...
		String [] columns = {"Año","ODS","Código de II.EE","Nombre de II.EE","Código de trabajo","Estado de trabajo","Categoría","Nivel de participación","Evaluador","Calificación"};
	...
	public ByteArrayInputStream reporteparticipantesinscritos(Integer ods,Integer anio,Integer categoria,Integer modalidad,Integer nivel,HttpSession ses)   {
		...
		String [] columns = {"Año","ODS","Departamento","Provincia","Distrito","Institución educativa","Código local II.EE","Fecha de registro","Inscrito al C.E","Nivel-N°Secciones-N°Docentes-N°Alumnos-N°Varones-N°Mujeres","Ámbito de II.EE","Modalidad de enseñanza", "Código local","Tipo de II.EE","Dirección","DRE","UGEL","Teléfono II.EE","Correo electrónico II.EE","Facebook","Lengua","Clasificación","Turno","Proveedor servicio","Horas abastecimiento","¿Piscina?","Tipo doc. director","N° doc. director","Apellidos del director","Nombres del director","Género del director","Teléfono del director","Celular del director","Correo del director","Tipo doc. docente","N° doc. docente","Apellidos del docente","Nombres del docente","Género del docente","Teléfono del docente","Celular del docente","Correo del docente","Nombres del docente inscrito","Apellido paterno del docente inscrito","Apellido materno del docente inscrito","Tipo doc. docente inscrito","N° doc. docente inscrito","Género del docente inscrito","Teléfono del docente inscrito","Correo del docente inscrito","Curso","Tipo de docente"};
		...
		String [] columns1 = {"Año","ODS","Departamento","Provincia","Distrito","Institución educativa","Código local II.EE","Fecha de registro","Inscrito al C.E","Nivel-N°Seciones-N°Docenes-N°Alumnos-N°Varones-N°Mujeres","Ámbito de II.EE","Modalidad de enseñanza","Código local","Tipo de II.EE","Dirección","DRE","UGEL","Teléfono II.EE","Correo electrónico II.EE","Facebook","Lengua","Clasificación","Turno","Proveedor de servicio","Horas de abastecimiento","¿Piscina?","Tipo doc. director","N° doc. director","Apellidos del director","Nombres del director","Género del director","Teléfono del director","Celular del director","Correo del director","Tipo doc. docente","N° doc. docente","Apellidos del docente","Nombres del docente","Género del docente","Teléfono del docente","Celular del docente","Correo del docente","Nombres del participante","Apellido paterno del participante","Apellido materno del participante","Fecha de nacimiento del participante","Sección del participante", "Tipo de doc. participante","N° doc. participante","Género del participante","Nivel del participante","Grado del participante", "Nivel de participación", "Modalidad", "Categorías","Nombre del tutor","Apellido paterno del tutor","Apellido materno del tutor","Teléfono de tutor","Correo del tutor", "Tipo doc. tutor","N° doc. tutor","Parentesco"};
		Sheet sheet1 = workbook.createSheet("Datos de participantes");
		Row row1 = sheet1.createRow(0);
		
		row1.createCell(0).setCellValue("DATOS DE LA II.EE");
	...
	public ByteArrayInputStream reportegeneral(Integer ods,Integer anio,Integer categoria,Integer modalidad,Integer nivel,HttpSession ses)   {
		...
		for(ResultadosRegionalesDto dto : listarr) {
			...
			row2.createCell(14).setCellValue(dto.getDocente());
			row2.createCell(15).setCellValue(dto.getCelularDocente());
			initRow2 ++;
		}
					...
					if(obj.getEstadotrabajo().getId().equals(21)) {
						...
						if(soloUnEmpate) {
							if(obj.getPuesto_nacional()!=null) {
								...
								puesto = obj.getPuesto_nacional().equals(1)?"PRIMER PUESTO":(obj.getPuesto_nacional().equals(2)?"SEGUNDO PUESTO":(obj.getPuesto_nacional().equals(3)?"TERCER PUESTO":""));
	...
	public ResponseEntity<InputStreamResource> exportParticipantes(@PathVariable(value="ods") String ods,
		...
		String [] columns = {"Nº","Año","ODS","Categoría","Entidad","Dirección","Comité técnico","Comité evaluador","Auspiciador","Aliado","Estado",
				"¿Cuántos contactos desea agregar?","Apellido paterno","Apellido materno","Nombres","Tipo de documento","N° de documento","N° de teléfono 1",
				"N° de teléfono 2","Correo institucional","Cargo","Apellido paterno","Apellido materno","Nombres","Correo institucional","Cargo","Usuario",
				"Contraseña","Enviar oficio","N° de oficio","Fecha de oficio","Monto total"};
		...
		String [] columns1 = {"ODS","N°","Descripción","Cantidad","Monto unitario","Monto total"};
